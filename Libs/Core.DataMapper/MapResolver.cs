    ...
    internal static class MapResolver
    {
                ...
                lock (mutex)
                {
                    if (!maps.TryGetValue(key, out map))
                    {
                        ...
                        foreach (var getProperty in getProperties)
                        {
                            ...
                            var setProperty = setProperties.Where(x => x.Name.Replace("_", "").ToUpper().Equals(getProperty.Name.Replace("_", "").ToUpper()))
                                                           .FirstOrDefault();
                            ...
                            if (setProperty != null && setProperty.CanWrite)
                            {
                                ...
                                //// add mapping
                                if (setProperty.IsDefined(typeof(ComplexAttribute)) || getProperty.IsDefined(typeof(ComplexAttribute)))
                                {
                                    map.Add<PropertyInfo, PropertyInfo>(getProperty, setProperty, true);
                                }
                                else
                                {
        ...
        private static IEnumerable<PropertyInfo> GetProperties(Type type, BindingFlags flags)
        {
            ...
            foreach (var property in properties)
            {
                ...
                if (!IsIndexerProperty(property) && !property.IsDefined(typeof(TransientAttribute)))
                {
