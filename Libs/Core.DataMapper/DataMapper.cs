        ...
        private static void Map(Map map, object source, object destination, Type sourceType, Type destinationType)
        {
            ...
            foreach (MapItem item in map.Items)
            {
                ...
                else
                {
                    ...
                    if (item.IsComplex)
                    {
                        var sourceItemCollection = sourceValue as IEnumerable<object>;
                        // complex object is not a list
                        if (sourceItemCollection == null)                        //// get resolved sub map between complex types
                        {
                        ...
                        // complex object is a list
                        else
                        {

                            System.Type specificType = typeof(List<>).MakeGenericType(new System.Type[] { item.Destination.Type.GetGenericArguments().Single() });
                            dynamic destItems = Activator.CreateInstance(specificType);

                            foreach (var i in sourceItemCollection)
                            {
                                Map subMap = new Map(i.GetType(), item.Destination.Type.GetGenericArguments().Single());

                                dynamic destValue = Map(i, item.Destination.Type.GetGenericArguments().Single());

                                destItems.Add(destValue);
                            }
                            destinationValue = destItems;
                        }
                    }
                    else
                    {
